...
const supportFields: ComponentDef[] = [];

sortBy(ComponentTypes, ["type"]).forEach((component) => {
    if (component.subType === "content") {
        contentFields.push(component);
    } else if (component.type === ComponentTypeEnum.DSIAccess) {
        supportFields.push(component);
            ...
            <ol className="govuk-list">
                <li className="component-create__list__item">
                    <h2 className="govuk-heading-s">
                        {i18n("component.supportfields_title")}
                    </h2>
                    <div className="govuk-hint">
                        {i18n("component.supportfields_info")}
                    </div>
                    <ol className="govuk-list">
                        {supportFields.map((component) => (
                            <li key={component.name}>
                                <a
                                    className="govuk-link"
                                    href="#0"
                                    onClick={(e) =>
                                        selectComponent(e, component)
                                    }
                                >
