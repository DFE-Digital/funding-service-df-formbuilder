...
import { MsalReactTesterPlugin, ITestRunner } from "./MsalReactTesterPlugins";
import {
...
class MsalReactTester {
    ...
    async waitForLogin() {
        await this._testRunner.waitingFor(() =>
            this._testRunner
                .expect(this._handleRedirectSpy)
                .toHaveBeenCalledTimes(1)
        );
        if (this.interationType === "Redirect")
            await this._testRunner.waitingFor(() =>
                this._testRunner
                    .expect(this._loginRedirectSpy)
                    .toHaveBeenCalledTimes(1)
            );
        else
            await this._testRunner.waitingFor(() =>
                this._testRunner
                    .expect(this._loginPopupSpy)
                    .toHaveBeenCalledTimes(1)
            );
    }
	...
	*/
    spyMsal() {
        ...
        this._testRunner
            ...
            .mockImplementation((account) => (this.activeAccount = account));
    }
    ...
    static GetNewClient = (
        ...
        return {
            ...
            handleRedirectPromise: () =>
                Promise.resolve(testAuthenticationResult),
