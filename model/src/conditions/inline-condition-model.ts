...
export class ConditionsModel {
    ...
    clone() {
        ...
        toReturn.#userGroupedConditions = this.#userGroupedConditions.map(
            (it) => it.clone()
        );
    ...
    _group(conditions, groupDefs) {
        return conditions.reduce((groups, condition, index, conditions) => {
            const groupDef = groupDefs.find((groupDef) =>
                groupDef.contains(index)
            );
            if (groupDef) {
        ...
        if (hasAnd && hasOr) {
            ...
            orPositions.forEach((position, index) => {
                ...
                if (thisIsTheLastOr && thereAreMoreConditions) {
                    groupDefs.push(
                        new GroupDef(position, conditions.length - 1)
                    );
                }
                start = position;
            });
...
class ConditionGroup {
    ...
    constructor(conditions) {
        if (!Array.isArray(conditions) || conditions.length < 2) {
            throw Error(
                "Cannot construct a condition group from a single condition"
            );
        }
...
class AbstractCondition {
    ...
    constructor(coordinator) {
        if (coordinator && !Object.values(coordinators).includes(coordinator)) {
            throw Error(
                `coordinator ${coordinator} is not a valid coordinator`
            );
        }
...
export class Condition extends AbstractCondition {
    ...
    constructor(field, operator, value, conditionType, coordinator, datasetId) {
        super(coordinator);
        ...
        if (typeof conditionType !== "string") {
            throw Error(`conditionType ${conditionType} is not valid`);
        }
        if (typeof datasetId !== "string") {
            throw Error(`datasetId ${datasetId} is not valid`);
        }
        this.field = field;
