...
import { get, post } from "@xgovformbuilder/model";
...
export class UploadService {
    ...
    async handleUploadRequest(
        ...
        for (const file of files) {
            ...
            const validFiles = (
                await Promise.all(
                    values.map(async (fileValue: any) => {
                        let fileTypes = [];
                        model.pages?.filter((page) => {
                            const fileUpload = page.pageDef.components?.filter(
                                (comp) => comp?.type == "FileUploadField"
                            );
                            fileUpload?.forEach((element) => {
                                fileTypes = element.addedFileTypes;
                            });
                        });
                        this.types = fileTypes.map((el: any) =>
                            el.toLowerCase()
                        );
                        const extension: any = fileValue.hapi.filename
                            .split(".")
                            .pop();
                        if (
                            !this.types.includes(
                            ...
                            request.pre.errors = [
                                ...
                                parsedError(
                                    key,
                                    `The selected file for "%s" must be a ${this.types
                                        .slice(0, -1)
                                        .join(", ")} or ${this.types.slice(-1)}`
                                ),
                            ];
                            return null;
                        }
