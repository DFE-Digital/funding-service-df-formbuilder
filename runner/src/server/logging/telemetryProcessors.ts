...
export function operationNameUUIDHider(): (
    envelope: Envelope,
    contextObjects?: any
) => boolean {
    return (envelope) => {
        ...
        if (envelope.tags && envelope.tags["ai.operation.name"]) {
            envelope.tags["ai.operation.name"] = hideUuidInUrlIfNotStaticFile(
...
export function errorLogger(
    logger: Logger
): (envelope: Envelope, contextObjects?: any) => boolean {
    return (envelope) => {
        if (
            envelope.data &&
            ...
            logger.info(
                `AppInsights error: ${JSON.stringify({
