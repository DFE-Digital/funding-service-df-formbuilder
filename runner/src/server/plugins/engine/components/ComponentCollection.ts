...
export class ComponentCollection {
    ...
    constructor(componentDefs: ComponentDef[] = [], model: FormModel) {
        const components = componentDefs.map((def) => {
            // Temporary code - remove after migration
            const dates = [
                "DateField",
                "TimeField",
                "DateTimeField",
            ...
            if (dates.includes(def.type)) def.type = "DateAndTimeField";
    ...
    getViewModel(
        ...
        let result;
        trackEvent(
            "ComponentCollection:getViewModel:start",
            {
                formData,
                errors,
        ...
        try {
            result =
                this.items?.map((item: any) => {
                    if (
                        formData?.tableData?.dataset === null &&
                        item?.type === "TableDataset"
                    )
                        return;
                    return {
                        type: item?.type,
            ...
            debugConsoleLog(error.message);
            trackEvent(
                "ComponentCollection:getViewModel:error",
                {
        ...
        if (conditions) {
            return result?.filter(
                (item) =>
                    conditions[item?.model?.condition]?.fn(formData) ?? true
            );
        }
