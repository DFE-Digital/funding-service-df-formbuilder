...
export class UkAddressField extends FormComponent {
    ...
    constructor(def: InputFieldsComponentsDef, model: FormModel) {
        ...
        const childrenList: any = [
            {
                ...
                schema: { max: 100 },
                options: { required: isRequired, optionalText },
            },
            {
                type: "TextField",
                name: "addressLine2",
        ...
        childrenList?.forEach(
            (child) => (child.name = `${name}__${child.name}`)
        );
    ...
    getFormDataFromState(state: FormSubmissionState) {
        ...
        if (value) {
            ...
            return {
                [`${name}__addressLine1`]: state[`${name}__addressLine1`],
                [`${name}__addressLine2`]: state[`${name}__addressLine2`],
                [`${name}__town`]: state[`${name}__town`],
                [`${name}__postcode`]: state[`${name}__postcode`],
            };
        }
    }
