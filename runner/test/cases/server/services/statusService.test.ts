...
suite("Server StatusService", () => {
  describe("shouldRetryPay", () => {
    ...
    test.skip("returns true when <3 pay attempts have been made", async () => {
      sinon
        .stub(cacheService, "getState")
    ...
    test.skip("returns true when <3 and the continue query is true", async () => {
      sinon
        .stub(cacheService, "getState")
        .returns({ pay: { meta: { attempts: 1 } } });
  ...
  describe("outputRequests", () => {
    ...
    test.skip("makes and returns correct output requests", async () => {
      ...
      stub
        ...
        .resolves("3");
      const statusService = new StatusService(server);
      const res = await statusService?.outputRequests({ yar, url: {hostname: "local"} });
      const results = await res.results;
      expect(res.reference).not.to.equal("abcd-ef-g");
  ...
  describe("renders custom text correctly", () => {
    ...
    test("with no customText defined", async () => {
      ...
      expect($("h1").text()).to.contain("Submission complete");
