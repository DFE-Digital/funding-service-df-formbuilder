...
describe("Title and section title", () => {
  it("does not render the section title if it is the same as the title", { timeout: 100000 },async () => {
    const options = {
      method: "GET",
    ...
    expect($("h1").text().trim()).to.startWith("");
  });
  it("does render the section title if it is not the same as the title",{ timeout: 100000 }, async () => {
    const options = {
      method: "GET",
    ...
    expect($("#section-title").text().trim()).to.be.equal("");
    expect($("h1.govuk-fieldset__heading").text().trim()).to.equal("");
  });
  it("renders the section title as H2, outside of the H1", { timeout: 100000 }, async () => {
    const options = {
      method: "GET",
      url: `/titles/applicant-one-address?visit=1`,
    };
