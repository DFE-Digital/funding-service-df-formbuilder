...
suite(`Server Config`, () => {
  ...
  test("it throws when MATOMO_URL is insecure", () => {
    ...
    expect(() => buildConfig()).to.throw(
      Error,
      "The server conf is invalid. Provided matomoUrl is insecure, please use https"
    );
  });
  ...
  test("it throws when PAY_API_URL is insecure", () => {
    ...
    expect(() => buildConfig()).to.throw(
      Error,
      "The server conf is invalid. Provided payApiUrl is insecure, please use https"
    );
  });
  ...
  test("it throws when PAY_RETURN_URL is insecure", () => {
    ...
    expect(() => buildConfig()).to.throw(
      Error,
      "The server conf is invalid. Provided payReturnUrl is insecure, please use https"
    );
  });
  ...
  test("it captures LAST_COMMIT and LAST_TAG environment variables", () => {
    ...
    expect(conf.lastCommit).to.equal("LAST COMMIT");
    expect(conf.lastTag).to.equal("LAST TAG");
  });
  ...
  test("it throws when oAuth conf is incomplete", () => {
    process.env = {
      ...process.env,
      ...customVariables,
      AUTH_ENABLED: true,
      ...
      AUTH_CLIENT_AUTH_URL: undefined,
      AUTH_CLIENT_TOKEN_URL: undefined,
      AUTH_LOGOUT_URL: undefined,
      API_URL: undefined,
      AUTH_REDIRECT_URL: undefined,
      AUTH_API_SECRET: undefined
    };

    expect(() => buildConfig()).to.throw(
      Error,
      'The server conf is invalid. "authClientId" is required. "authClientSecret" is required. "authClientAuthUrl" is required. "authClientTokenUrl" is required. "authRedirectUrl" is required. "authApiSecret" is required. "authLogoutUrl" is required'
    );
  });

  test("it captures oAuth conf", () => {
    process.env = {
      ...process.env,
      ...customVariables,
      AUTH_ENABLED: "true",
      ...
      AUTH_REDIRECT_URL: "redirect url",
      AUTH_API_SECRET: "apisecret",
      API_URL: "url",
      AUTH_LOGOUT_URL: "oAuth logout url",
    };

    const conf = buildConfig();
    expect(conf.authEnabled).to.equal(true);
    expect(conf.authClientAuthUrl).to.equal("oAuth auth url");
    expect(conf.authClientTokenUrl).to.equal("oAuth token url");
    expect(conf.authClientId).to.equal("oAuth client ID");
    expect(conf.authClientSecret).to.equal("oAuth client secret");
    expect(conf.authLogoutUrl).to.equal("oAuth logout url");
  });
