...
suite("Condition Evaluation Context", () => {
  it("it correctly includes/filters state values", () => {
    ...
    expect(completeState.applicantOneDetails.firstName).to.equal("Martin");
    expect(completeState.applicantOneDetails.lastName).to.equal("Crawley");
    expect(completeState.applicantOneDetails.address).to.equal({
      addressLine1: "AddressLine1",
      addressLine2: "AddressLine2",
      town: "Town",
      postcode: "Postcode",
    ...
    expect(completeState.checkBeforeYouStart.ukPassport).to.equal(false);
    expect(completeState.applicantOneDetails).to.exist();
    expect(completeState.applicantDetails).to.equal({
      numberOfApplicants: 1,
      phoneNumber: "1234567890",
      emailAddress: "developer@example.com",
    });
