...
suite("CheckboxesField", () => {
  describe("Generated schema", () => {
    ...
    it("verify view model", () => {
      ...
      expect(viewModel.name).to.equal("myCheckbox")
    });

    it("verify display string", () => {
      const displayString = component.getDisplayStringFromState({
        myCheckbox: ["3", "4"]
      });
      expect(displayString).to.equal("3, 4")
    });
  });
});
