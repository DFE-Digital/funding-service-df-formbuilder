...
suite(`Phase banner`, () => {
  ...
  test("shows the beta tag by default", { timeout: 100000 }, async () => {
  ...
  test("shows the alpha tag if selected", { timeout: 100000 }, async () => {
    server = await createServer({
  ...
  test("does not show the phase banner if None",{ timeout: 100000 }, async () => {
    server = await createServer({
